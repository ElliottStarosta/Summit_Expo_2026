...
export function WaypointEditor() {
  ...
  const onPanelMove = (e: React.PointerEvent) => {
    ...
    setPanelPos({
      x: Math.max(
        0,
        panelDragRef.current.ox + e.clientX - panelDragRef.current.sx,
      ),
      y: Math.max(
        0,
        panelDragRef.current.oy + e.clientY - panelDragRef.current.sy,
      ),
    });
  };
  const onPanelUp = () => {
  ...
  const addHandle = (idx: number) => {
    setAnchors((p) =>
      p.map((a, i) =>
        i === idx
          ? { ...a, cpxPct: a.xPct + 0.1, cpyPct: Math.max(0, a.yPct - 0.1) }
          : a,
      ),
    );
  };
    ...
    <>
        ...
        {resolved.map((p, i) => {
          ...
          return (
            <g key={`n${i}`} style={{ pointerEvents: "all" }}>
              ...
              {p.cpx != null &&
                hcy != null &&
                Math.abs(hcy) < window.innerHeight + 80 && (
                  <circle
                    cx={p.cpx}
                    cy={hcy}
                    r={7}
                    fill="rgba(55,145,255,0.88)"
                    stroke="#fff"
                    strokeWidth={1.5}
                    className="wpe-node-hit"
      ...
      >
        ...
        <div className="wpe-howto">
          Drag nodes on the page · nodes snap to section % positions.
          <br />
              ...
              >
                ...
                <div className="wpe-item-top">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      e.stopPropagation();
                      changeSelector(i, e.target.value);
                    }}
                ...
                <div className="wpe-pct-row">
                  ...
                  <span className="wpe-pct-val">
                    {(a.xPct * 100).toFixed(1)}%
                  </span>
                  <span className="wpe-pct-label">Y</span>
                  <span className="wpe-pct-val">
                    {(a.yPct * 100).toFixed(1)}%
                  </span>
                ...
                {a.cpxPct != null ? (
                  <div className="wpe-handle-row">
                    ...
                    <span className="wpe-pct-val">
                      {(a.cpxPct * 100).toFixed(1)}%
                    </span>
                    <span className="wpe-pct-val">
                      {(a.cpyPct! * 100).toFixed(1)}%
                    </span>
                    <button
